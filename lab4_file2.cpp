#include<iostream>
using namespace std;
class node{                //creating node to store data
	public:
	float data; 
	node* next;
};
class stackll{             //creating stack
	public:
	node* bottom, *top; 
	stackll(){
		bottom = NULL;
...
class queue{        //queue using 2 stack
...
void stackll::mkqu(node* bottom){                                  //pushing into another stack in reverse order
		node* temp =bottom;
		if(bottom!=NULL){
